...
if (isset($firstName) && isset($lastName) && isset($city) && isset($state) && isset($phone) && isset($password) && isset($passwordConfirm) && isset($describeYourself)){
					...
					if ($locationInfoCheck != "good"){
						...
						if ($passwordCheck != "good"){
							...
							if ($passwordCheck != "good"){
								...
								process_worker($firstName, $lastName, $city, $state, $phone, $email, $password, $legalWork, $transpt, $describeYourself, $available, $newPassword, $redirUrl);
							}
						}
					}
				}
...
function process_worker($firstName, $lastName, $city, $state, $phone, $email, $password, $legalWork, $transpt, $describeYourself, $available, $newPassword, $redirUrl){
	
	
	//////////
	$describeYourself = str_replace("&","and",$describeYourself);		
	$curl_connection = curl_init('https://job-circus.com/processEditWorker2.php');
	...
	curl_setopt($curl_connection, CURLOPT_REFERER, 'processEditWorker.circus');
