...
function display_joblistings($city,$state,$genre){ 
	...
	//begin the query 
	$sql = ('SELECT * FROM `Jobs` WHERE `city` = \''.$city.'\' AND `state` = \''.$state.'\' AND `genre` = \''.$genre.'\' AND `pay` = \'yes\' ORDER BY `wage` DESC');
	//echo $sql;
	...
	if ($rows<=0 ){ 
		...
		while ($row= mysql_fetch_array($sql_result)) {
			...
			echo("\n					");
			echo("<p class=\"jobListingList\"><a href=\"displayEmployerProfile.php?eid=".$employerID."\">".$wage.$tips." - ".$businessName." - ".$position."</a></p>");
  		}
		echo("\n");	
	}		
}
...
$showJobs = 0;
if (isset($_POST['state']) && isset($_POST['city']) && isset($_POST['genre'])){
	//ready to show jobs
	$showJobs = 1;
} else {
	//redirect
		...
		<div id="pageContent" align="center">
			<p>
				<div id="middleProfileInfo" >
					<?php
						if($showJobs == 1){
							display_joblistings(($_POST['city']),($_POST['state']),($_POST['genre']));
						}
					?>
					<p>
						*Plus tips
